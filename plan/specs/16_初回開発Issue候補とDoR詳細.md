...
| 候補ID | 対応機能 | 優先度 | 目的 |
| --- | --- | --- | --- |
| DEV-001 | F-1301 / F-1303 | P0 | Run状態遷移と復帰手順を先に固定し、以降機能の実行基盤を確定する |
| DEV-002 | F-1305 / F-1306 | P0 | 例外4項目と品質ゲート判定を連動させ、危険な実行を先に止められる状態を作る |
| DEV-003 | F-1307 / F-1311 | P0 | `run_id` 完全性検証と GitHub App(bot) PR作成を接続し、成果物の出口を確立する |
...
1. DEV-001（状態遷移の土台）
2. DEV-002（ポリシー/品質ガード）
3. DEV-003（成果物出口と監査）

## 7. 未決事項

- 候補ID（DEV-001〜003）を実際のIssue番号へ変換する命名規約。
- 各候補の担当アサイン（PM固定か、実装担当を別途割当てるか）。
