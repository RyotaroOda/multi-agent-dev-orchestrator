...
- 受け入れ条件:
  ...
  - `skip` 記法やテスト削減閾値違反時は、例外ラベル有無に関係なく `blocked` になる。
...
- テスト観点:
  ...
  - 異常系: 失効日判定不可またはポリシー判定不能時に fail-closed（`deny` + `blocked`）になる。
  - 異常系: 品質ルールエンジン失敗時に fail-closed で `blocked` になる。
- 関連リスク:
  - `R-003`, `R-005`, `R-015`
- DoRレビュー結果（2026-02-10）:
  - 充足（不足追記なし）
