...
### FR-606 判定境界マッピング（v0.2固定）

`deny`/`blocked` 境界を以下で固定し、`05`/`11`/`15` と同一語彙で運用する。

| 入力条件 | `policy_result` | `quality_result` | `run_control` | 備考 |
| --- | --- | --- | --- | --- |
| 例外4項目（理由/適用範囲/失効日/承認者）の欠落 | `deny` | `blocked` | `blocked` | fail-closed |
| 失効日判定不可 / policy判定エンジン異常 | `deny` | `blocked` | `blocked` | fail-closed |
| `skip/only` 記法追加 | `allow` | `blocked` | `blocked` | 例外適用不可 |
| アサーション減少閾値違反 | `allow` | `blocked` | `blocked` | 例外適用不可 |
| テストファイル削除 | `allow` | `blocked` | `blocked` | 例外適用不可 |
| ルールエンジン失敗 | `allow` | `blocked` | `blocked` | fail-safe |
| テスト実行コマンド改変 + `policy:allow-test-change` 有効 | `allow` | `pass` | `continue` | 4項目充足が前提 |
| CI設定改変 + `policy:allow-ci-change` 有効 | `allow` | `pass` | `continue` | 4項目充足が前提 |

### FR-607 例外可否ケース表（v0.2固定）

| ケース | 例外適用可否 | 判定 |
| --- | --- | --- |
| `skip/only` 記法追加 | 不可 | `blocked` |
| アサーション減少閾値違反 | 不可 | `blocked` |
| テストファイル削除 | 不可 | `blocked` |
| テスト実行コマンド改変 | 可（`policy:allow-test-change`） | 条件充足時のみ `pass` |
| CI設定改変 | 可（`policy:allow-ci-change`） | 条件充足時のみ `pass` |
