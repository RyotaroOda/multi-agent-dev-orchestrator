...
- `状態`: `queued` / `running` / `blocked` / `completed`
...
| TR-1804 | `blocked` | `queued` | 復帰要求が権限内、復帰可否の人間判断コメントあり、新 `run_id` 採番可能 | `previous_run_id`, `new_run_id`, `retry_reason`, `requested_by` | `blocked` 維持 + `blocked_reason=retry_condition_unmet` |

補足:
- `TR-1804` は「復帰受付」の遷移であり、直後に `TR-1801` で再実行に入る。
...
- `completed -> running`
- `completed -> queued`
- `blocked -> running`（`queued` を経由しない直接復帰）
- `queued -> completed`
...
| CK-1803 | 復帰再採番 | `blocked` 復帰時に必ず新 `run_id` が発行される |
| CK-1804 | 復帰前提検証 | 人間判断コメント欠落時は `retry_condition_unmet` を記録して復帰拒否する |
| CK-1805 | 語彙整合 | `blocked_reason` が本仕様と `15` の定義語彙のみで構成される |
