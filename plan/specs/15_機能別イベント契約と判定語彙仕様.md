...
| F-1305 | 実行イベント、例外申請データ（4項目） | `allow/deny`、`run_control`（continue/blocked）、拒否理由コード | 判定不能時 fail-closed（`policy_result=deny` かつ `run_result=blocked`） | `05_セキュリティポリシー運用仕様.md` |
...
- `blocked_reason` の最終命名規約（snake_case 固定/接頭辞有無）。
- `timestamp` の正規フォーマット（UTC固定のISO8601表記ルール）。
- Run Header の記録フォーマット（YAML固定かJSON固定か）。
