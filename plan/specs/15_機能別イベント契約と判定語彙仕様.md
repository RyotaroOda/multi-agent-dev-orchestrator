...
| F-1305 | 実行イベント、例外申請データ（4項目） | `allow/deny`、`run_control`（continue/blocked）、拒否理由コード | 判定不能時 fail-closed（`policy_result=deny` かつ `run_control=blocked`） | `05_セキュリティポリシー運用仕様.md` |
| F-1306 | CI結果、差分情報、例外ラベル情報 | `pass/blocked`、根拠付き判定結果、例外監査項目（`approver`, `expiry_date`, `scope`） | ルールエンジン失敗時 `blocked`、理由出力 | `06_品質ゲートと自動マージ仕様.md` |
...
## 9. DEV-002 判定語彙マッピング（`05`/`06`/`11` 同期）

| 入力条件 | `policy_result` | `quality_result` | `run_control` | `blocked_reason` |
| --- | --- | --- | --- | --- |
| 例外4項目欠落（理由/適用範囲/失効日/承認者） | `deny` | `blocked` | `blocked` | `policy_denied` |
| 失効日判定不可 / policy判定エンジン異常 | `deny` | `blocked` | `blocked` | `policy_denied` |
| `skip/only` 記法追加 | `allow` | `blocked` | `blocked` | `quality_rule_violation` |
| アサーション減少閾値違反 | `allow` | `blocked` | `blocked` | `quality_rule_violation` |
| テストファイル削除 | `allow` | `blocked` | `blocked` | `quality_rule_violation` |
| ルールエンジン失敗 | `allow` | `blocked` | `blocked` | `quality_rule_violation` |
| テスト実行コマンド改変 + `policy:allow-test-change` 有効 | `allow` | `pass` | `continue` | なし |
| CI設定改変 + `policy:allow-ci-change` 有効 | `allow` | `pass` | `continue` | なし |

補足:
- fail-closed は `policy_result=deny` かつ `run_control=blocked` を必須とする。
- `skip/only`、アサーション減少閾値違反、テストファイル削除は例外ラベル適用不可。

## 10. 監査項目（承認者/失効日）の出力契約

| 項目 | 出力先1 | 出力先2 | 追跡キー |
| --- | --- | --- | --- |
| `approver` | Issueコメント | 構造化ログ | `run_id` |
| `expiry_date` | Issueコメント | 構造化ログ | `run_id` |
| `scope` | Issueコメント | 構造化ログ | `run_id` |

運用ルール:
- 例外適用時は `approver` と `expiry_date` の両方を必須出力とする。
- 出力欠落時は `run_control=blocked` とし、補完完了まで次Runを開始しない。
