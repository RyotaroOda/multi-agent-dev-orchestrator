...
- `policy:allow-test-change` / `policy:allow-ci-change`: PM承認を必須。
- 緊急適用時は、`policy:allow-test-change` / `policy:allow-ci-change` に対して24時間以内のセキュリティ担当事後追認コメントを必須。

## 4. 運用フロー

1. 申請者が Issue コメントで例外申請
2. 承認者要件（3.1）を満たす承認コメントを記録（承認者名・承認理由・失効日を記載）
3. ラベル付与後に Run 実行
...
- 出力先:
  - Issueコメント（人間確認用）
  - 構造化ログ（機械集計用）
- `承認者` と `失効日` は両出力先で必須とする。
- いずれかの出力先で `承認者` または `失効日` が欠落した場合、当該Runは `blocked` とし次回Run開始前に補完コメントを必須化する。
