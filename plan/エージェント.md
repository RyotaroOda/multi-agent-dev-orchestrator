...
## 15. 個人xマルチエージェント運用プロファイル

### 15.1 運用目的

* 1人開発で発生しやすい「判断渋滞」「台帳更新遅延」「文脈切替過多」を抑える
* 複数ロールを使っても、Developer の認知窓口は PM Agent に固定する

### 15.2 稼働モード

| モード | 条件 | 運用 |
| --- | --- | --- |
| Focus | 高優先度を1件で進める | `running=1` を維持し、設計->実装->品質を直列で処理 |
| Parallel | 待ち時間を吸収したい | `running<=2` で独立Issueのみ並列 |
| Recovery | `blocked` 復帰判断が必要 | 新規 `queued` を止め、復帰条件の確認を優先 |

### 15.3 個人運用のWIP制約

* 同時 `running` は最大2件
* 24時間超の `blocked` が存在する間は新規着手を停止
* 上限超過の例外は ADR 記録を必須とする
...
* `allow/deny/blocked` と `blocked_reason` の語彙一致
...
* プランニング時は `plan/templates/個人xマルチエージェント_TaskBriefテンプレート.md` で入力条件を固定する
* PR提出/レビュー時は `plan/templates/個人xマルチエージェント_PRチェックリスト.md` を適用する
* `Commit -> PR -> Merge -> Done` の全体フローは `plan/specs/20_GitHub前提コミットPRマージ完了運用仕様.md` を参照する
* 起動時コンテキスト管理は `plan/specs/21_起動時コンテキストファイル運用仕様.md` を参照する
* 週次のGitHub保護監査は `plan/specs/22_AI監査自動化運用仕様.md` を参照し、AI監査 + 人間最終承認で運用する
* 監査ジョブの実装分解は `plan/specs/23_AI監査ジョブ実装詳細仕様.md` を参照する
* 実装着手前は `plan/specs/24_開発環境構築仕様.md` のチェックを先に実施する
